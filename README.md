...
# Configs
Inside the ``config.ini`` you'll find the MAC address field that should contain the MAC address that the packet is targetting (yours in this case, depending on what interface you're using).
...
# Logging
This service registers a logging section in the Windows event logs, so you can watch there for when it does something or errors out due to a missing config file.
